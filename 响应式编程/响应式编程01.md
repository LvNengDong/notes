...
public interface Publisher<T> {
    ...
    该方法会在订阅者订阅的事件发生后被触发，触发后向所有的订阅者
    */
...
- 其中，`onSubscribe()` 从名字上就能看出是一个回调方法，当发布者调用 `subscribe(Subscriber)` 方法时，接着会执行 Subscriber 对象的 `onSubscribe(Subscription)` 方法。
- 在 `onSubscribe(Subscription)`  方法中有一个参数 Subscription，这个 Subscription 可看作是一种用于订阅的上下文对象，Subscription 对象中包含了订阅者本次向发布者请求的数据个数。
