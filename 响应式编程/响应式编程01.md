响应式编程是一种新的编程技术，其目的是构建响应式系统。对于响应式系统而言，任何时候都需要确保系统具备即时响应性，这是大多数日常业务场景所需要的，但却是一项非常复杂而有挑战性的任务，需要对相关技术体系有深入的了解。
...
数据流对于技术栈而言是一个全流程的概念。也就是说，无论是从底层数据库，向上到达服务层，最后到 Web 服务层，亦或是在这个流程中所包含的任意中间层组件，整个数据传递链路都应该是采用事件驱动的方式来进行运作的。
...
什么是背压？简单来说就是`下游能够向上游反馈流量请求的机制`。通过前面的分析，我们知道如果消费者消费数据的速度赶不上生产者生产数据的速度时，生产者就会持续消耗系统的资源（将新的节点添加到队列中），直到这些资源被消耗殆尽。
...
public interface Publisher<T> {
    /*
    Publisher就是监听器模式中的发布者，它会持有所有的订阅者实例。
    该方法会在订阅者订阅的事件（event）发生后被调用，调用后向所有
    的订阅者发送回调
    */
...
Subscriber 表示一种可以**向发布者订阅事件**并**接收订阅事件结果**的角色。`Subscriber<T>` 的接口定义如下。
...
- 其中，`onSubscribe()` 从名字上就能看出是一个回调方法，当发布者调用 `subscribe(Subscriber)` 方法时，接着会在方法体中执行 Subscriber 对象的 `onSubscribe(Subscription)` 方法。
- 在 `onSubscribe(Subscription)`  方法中有一个参数 Subscription，Subscription 对象可看作是一种用于订阅的上下文对象，Subscription 对象中包含了订阅者本次向发布者请求的数据个数。
- 当知道了请求数据的个数后，订阅者就可以调用 `onNext()` 方法向发布者请求得到一份数据。这个过程是持续不断的，直到达到订阅者所请求数据的总量。
- 此时，订阅者就会执行 `onComplete()` 方法，代表已经完成了从发布者获取数据的过程。
- 而一旦在这个过程中出现了异常，就会触发 `onError()` 方法，我们可以通过这个方法捕获到具体的异常信息进行处理，而数据流也就自动终止了。
...
>> 发布者通过subscribe向订阅者发送数据 
>> 订阅者通过onNext方法向发布者请求数据，直到完成预定的个数 
>> 订阅者接收完成会执行onComplete方法，异常则执行onError方法，数据流也会停止。
```
